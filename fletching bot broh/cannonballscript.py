...
def main():
    def main():
        print('20 seconds to start')
        time.sleep(20)
        while True:
            click_image("images/bankbooth.png", imagesearch_loop("images/bankbooth.png", .2), "left",
                        random.uniform(.15, .35), offset=random.uniform(10.0, 20.0))
            click_image("images/steelbar.png", imagesearch_loop("images/bankdepositbows.png", .2), "left",
                        random.uniform(.15, .35),
                        offset=10)

            click_image("images/bankclose.png", imagesearch_loop("images/bankclose.png", .2), "left",
                        random.uniform(.15, .35),
                        offset=3)
            click_image("images/furnace.png", imagesearch_loop("images/bowinv.png", .2), "left",
                        random.uniform(.15, .35),
                        offset=7)
            click_image("images/bowstringinv.png", imagesearch_loop("images/bowstringinv.png", .2), "left",
                        random.uniform(.15, .35),
                        offset=7)
            click_image("images/makeballs.png", imagesearch_loop("images/makebow.png", .2), "left",
                        random.uniform(.15, .35),
                        offset=20)
            placeholder = imagesearch("images/emptyinventory.png", precision=.95)
            counter = 0
            while placeholder[0] == -1:
                placeholder = imagesearch("images/emptyinventory.png", precision=.95)
                if placeholder[0] != -1:
                    break
                counter += 1
